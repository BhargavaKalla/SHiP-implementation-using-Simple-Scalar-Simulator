	     ...
	     
{
  ...
  /*Initialising extra parameters required for implementing BIP and DIP*/                  
  cp->PSEL_counter = 0;
  cp->PSEL_width = 10;
  cp->BIP_counter = 0;
  cp->BIP_width = 5;
  
  /*Initialising BRRIP counter for implementing BRRIP and DRRIP*/
  cp->BRRIP_counter = 0;
  ...
  for (bindex=0,i=0; i<nsets; i++)
  {
      ...
      /* If the policy is DIP then divide the 1024 sets into 32 regions of 32sets each and use the set dueling process as described in the assignment 
        and dedicate 1 out of 32 sets to LRU, 1 out of 32 sets to BIP and remaining sets as follower */
	    if(cp->policy == DIP)                   
      {
		    if ( i % ((nsets/32)+1) == 0 ) 
		    {
			    cp->sets[i].dynamic_set_type = lru;
         
		    }
            
		    else if ( (i+1) % ((nsets/32)-1) ==  0) 
		    {
			    cp->sets[i].dynamic_set_type = bip;
          
		    } 
		    else 
		    {
			    cp->sets[i].dynamic_set_type = follower;
          
		    }
	    }
         
       
      /* If the policy is DRRIP then divide the 1024 sets into 32 regions of 32sets each and use the set dueling process as described in the assignment 
        and dedicate 1 out of 32 sets to SRRIP, 1 out of 32 sets to BRRIP and remaining sets as followerset */
      if(cp->policy == DRRIP)                   
      {
		    if ( (i+1) % ((nsets/32)-1) ==  0)
		    {
			    cp->sets[i].drrip_set_type = brrip;
          
		    }
		    else if ( i % ((nsets/32)+1) == 0 ) 
		    {
			    cp->sets[i].drrip_set_type = srrip;
         
		    }
		    else 
		    {
			    cp->sets[i].drrip_set_type = followerset;
      ...
      for (j=0; j<assoc; j++)
	    {
         ...
         /* RRPV counter for each is initialised to 3 */     
        blk->RRPV_counter = 3;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case LIP:
    ...
    /*replace the tail block and insert the incoming block in the LRU position i.e., Tail*/
    repl = cp->sets[set].way_tail;
  ...
  case BIP:
    ...
    /*replace the tail block and  i.e., Tail*/
    repl = cp->sets[set].way_tail;
    
    /*bimodal throttle parameter, e = 1/32, insert the incoming block in the MRU position if BIP_counter is 1 else in all the other 31 cases insert in the LRU position*/
    if (cp->BIP_counter == 1) 
    {
		    update_way_list(&cp->sets[set], repl, Head);      
    }
    ...
    /*Increment the BIP counter and set to 0 if it  reaches its maximum value */ 
    int  BIP_max = 1<<cp->BIP_width;
    if (cp->BIP_counter < BIP_max) 
    {
	    cp->BIP_counter++;
    }
    else 
  ...
  case DIP:
    ...
    /*If the dedicated set type is of the type LRU then insert the incoming block in the MRU position*/                       
    if (cp->sets[set].dynamic_set_type == lru) 
    {
    	update_way_list(&cp->sets[set], repl, Head);
      
      /* Increment the Policy selection counter */
      int PSEL_max = 1<<cp->PSEL_width;
	    if (cp->PSEL_counter < PSEL_max) 
	    {
	      cp->PSEL_counter++;
	    }
    }
    ...
    /*If the dedicated set type is of the type BIP then follow the BIP policy as described in the BIP case*/    
    else if (cp->sets[set].dynamic_set_type == bip) 
    {
     if (cp->BIP_counter == 1) 
	    {
		    update_way_list(&cp->sets[set], repl, Head);
      ...
      /* Decrement the Policy selection counter */   
    ...
    /*If the dedicated set type is of the type follower then look at the MSB of the policy selection counter*/   
    else 
    { 
	    int PSEL_MSB = cp->PSEL_counter >> (cp->PSEL_width-1);
         
      /* If the MSB of the policy selection counter is 0 then follow LRU policy*/     
	    if ( PSEL_MSB == 0 ) 
	    {		    
        update_way_list(&cp->sets[set], repl, Head);
	    }
         
      /* If the MSB of the policy selection counter is not zero then follow BIP policy*/     
	    else 
	    {
		    if (cp->BIP_counter == 1) 
        {
     ...
     /*Increment the BIP counter and set to 0 if it  reaches its maximum value */ 
    int  BIP_maximum = (1<<cp->BIP_width)-1;		
    if (cp->BIP_counter < BIP_maximum) 
    {
	    cp->BIP_counter++;
    }
    else 
  ...
  case SRRIP:
    ...
    /*Initialise the replacing block with the left most block */ 
    repl = cp->sets[set].way_head;
    
    /* Starting from the left search for a block whose RRPV counter value is 3 */ 
    for (blk=cp->sets[set].way_head; blk; blk=blk->way_next) 
    { 
    
       /* If the block is found then replace the block and set its RRPV counter value as 2*/ 
      if(blk->RRPV_counter == 3)                 
      { 
        repl = blk;
        
        /* set the RRPV counter of the replaced block as 2*/ 
        repl->RRPV_counter = 2;
        break; 
      }       
      
      /* If the block is not found,then increment RRPV counter values of all the blocks.  */ 
    ...
    case BRRIP:
        ...
        { 
          ...
          /* If the block is found then replace the block*/
          if(blk->RRPV_counter == 3)                
          { 
            repl = blk;
            
            /*bimodal throttle parameter, e = 1/32, if BIP_counter is 1 then replace the block and set its RRPV counter value as 2 */
                                                  
            if(cp->BRRIP_counter == 1) 
            {
              repl->RRPV_counter = 2 ;      
            }
            
            /*In all the other 31 cases, set its RRPV counter value as 2*/
            else
            {  
              repl->RRPV_counter = 3; 
            }
    
            /*Increment the BRRIP counter and set to 0 if it  reaches its maximum value */ 
            if (cp->BRRIP_counter < 32) 
            {
    	        cp->BRRIP_counter++;
            }
  ...
  case DRRIP: 
    ...
    { 
      ...
      { 
        ...
        /*If the dedicated set type is of the type LRU then insert the incoming block in the MRU position*/
        if(cp->sets[set].drrip_set_type == srrip) 
        {
          repl->RRPV_counter = 2;
          
          /* Increment the Policy selection counter */ 
          if (cp->PSEL_counter < 1024) 
	        {
	          cp->PSEL_counter++;
	        }  
        }     
    
        /*If the dedicated set type is of the type BRRIP then follow the BRRIP policy as described in the BRRIP case*/   
        else if(cp->sets[set].drrip_set_type == brrip)
        {  
            if(cp->BRRIP_counter == 1) 
            {
              repl->RRPV_counter = 2 ;      
        ...
        /*If the dedicated set type is of the type followerset then look at the MSB of the policy selection counter*/
        else 
        {
        
            /* If the MSB of the policy selection counter is 0 then follow SRRIP policy*/
            int PSEL_MSB = cp->PSEL_counter >> (cp->PSEL_width-1);
            if ( PSEL_MSB == 0 ) 
            {
              repl->RRPV_counter = 2;
            ...
            /* If the MSB of the policy selection counter is not zero then follow BRRIP policy as described in the BRRIP case */
            else
            {
              if(cp->BRRIP_counter == 1) 
              {  
  ...
  case SHiP:  
    ...
    signature = (int)addr >> 18;
    signature_m = signature;
 ...
 cache_hit: /* slow hit handler */
  ...
  if ((strcmp(cp->name, "ul2") != 0) || (cmd == Read)) {
    ...
    /* if LIP and this is not the first element of list, reorder */
    ...
    /* if BIP and this is not the first element of list, reorder */            
  ...
  /* if DIP and this is not the first element of list, reorder */             
    ...
    /*If there is a hit and the policy is SRRIP then update RRPV to 0*/
    if(blk->way_prev && cp->policy == SRRIP) 
    {                                               
      blk->RRPV_counter = 0; 
    }
    
     /*If there is a hit in BRRIP then update RRPV to 0*/
    if(blk->way_prev && cp->policy == BRRIP) 
    {                                            
      blk->RRPV_counter = 0; 
    }
    
     /*If there is a hit in DRRIP then update RRPV to 0*/
    if(blk->way_prev && cp->policy == DRRIP) 
    {                                
      blk->RRPV_counter = 0; 
    }
    ...
    if (cp->policy == SHiP)
    {
      /* move this block to head of the way (MRU) list */
       ...
       signature_m = signature;
       blk->outcome = 1;                       //outcome is true 
       SHCT[signature_m]++;               // decrement SHCT[signature]
      // printf("%d\n", SHCT[blk->signature_m]);
       blk->RRPV_counter = 0;
       
       if(SHCT[signature_m]  >= 7 );
       {
          SHCT[signature_m] = 7;
       }  
    }
